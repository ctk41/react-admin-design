...
const VideoWatermark: FC = () => {
  ...
  const handleAddText = () => {
    ...
    if (elements.length > 4) {
      message.warning('A maximum of 5 elements can be layered on the image!');
      return;
    } else {
      setElements(draft => {
  ...
  return (
    <PageWrapper plugin={VIDEO_WATERMARK}>
      <Row gutter={12}>
        <Col span={16}>
          <Card title='Composition Area' bordered={false} bodyStyle={{ height: '550px' }}>
        ...
        <Col span={8}>
          <Card title='Settings Area' bordered={false} bodyStyle={{ height: '550px' }}>
            <Form
              colon={false}
              labelCol={{ span: 6 }}
              wrapperCol={{ span: 18 }}
            ...
            >
              <Form.Item label='Select Video'>
                <Button
                  type='primary'
                  style={{ width: '100%' }}
                  onClick={() => message.warning('Please configure the video resource service interface!')}
                >
                  Select Video
                </Button>
              </Form.Item>
